    ...
    public class ResumeParserService : IResumeParserService
    {
        ...
        public async Task<ResumeAnalysis> ParseResumeAsync(IFormFile file)
        {
            ...
            var analysis = new ResumeAnalysis
            {
                ...
                Education = education,
                PersonalSummary = SummarizePersonalInfo(name, email, phone, linkedIn, "")
            };
        ...
        private string SummarizePersonalInfo(string name, string email, string phone, string linkedIn, string location)
        {
            var summaryParts = new List<string>();
            if (!string.IsNullOrWhiteSpace(name)) summaryParts.Add($"Name: {name}");
            if (!string.IsNullOrWhiteSpace(email)) summaryParts.Add($"Email: {email}");
            if (!string.IsNullOrWhiteSpace(phone)) summaryParts.Add($"Phone: {phone}");
            if (!string.IsNullOrWhiteSpace(location)) summaryParts.Add($"Location: {location}");
            if (!string.IsNullOrWhiteSpace(linkedIn)) summaryParts.Add($"LinkedIn: {linkedIn}");
